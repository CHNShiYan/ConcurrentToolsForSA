...
public class task_cReducer extends Reducer<Text, Text, Text, Text>{
	...
	protected void reduce(Text key, Iterable<Text> values,
		...
		while(iterator.hasNext()){
			String[] house_type_price=iterator.next().toString().split(" ");
			String house_type=house_type_price[0];
			Double house_price=Double.parseDouble(house_type_price[1]);
			if (cnt.containsKey(house_type))  
				cnt.put(house_type, cnt.get(house_type) + 1);  
	        else
	           	cnt.put(house_type, 1);  
			if (sum.containsKey(house_type))  
				sum.put(house_type, sum.get(house_type) + house_price);  
	        else
	        	sum.put(house_type, house_price);
		}
